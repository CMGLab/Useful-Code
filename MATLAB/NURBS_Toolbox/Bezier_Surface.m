function [] = Bezier_Surface(d,p_1,p_2,n_1,n_2,Xi_1,Xi_2,P,w,h,varargin)
...
elseif d == 3
    %figure
    surf(S(:,:,1),S(:,:,2),S(:,:,3),'EdgeAlpha',0,'FaceColor',[2/3 2/3 2/3])
    xlabel('X')
    ylabel('Y')
    zlabel('Z')
    ...
    plot3(S(1,:,1),S(1,:,2),S(1,:,3),'k')
    plot3(S(end,:,1),S(end,:,2),S(end,:,3),'k')
    plot3(S(:,1,1),S(:,1,2),S(:,1,3),'k')
    plot3(S(:,end,1),S(:,end,2),S(:,end,3),'k')
